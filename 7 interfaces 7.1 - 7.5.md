...
接口类型（interface types）表达了关于其他类型行为的泛化（generalizations）或抽象（abstractions） 。 通过泛化，接口使我们能够编写更为灵活和适应性更强的函数，因为它们不依赖于某一特定实现的细节 。 许多面向对象的语言都有某种接口的概念，但Go语言接口的显著特点在于它们是被**隐式满足**的（satisfied implicitly） 。 换句话说，无需声明一个给定的具体类型（concrete type）满足了哪些接口； 仅仅拥有必要的方法就足够了 。 这种设计允许你创建新的接口，这些接口可以被现有的具体类型满足而无需改变现有类型，这对于那些定义在你无法控制的包中的类型尤其有用 。

在本章中，我们将首先考察接口类型及其值的基本机制 。 在此过程中，我们将学习标准库中的几个重要接口 。 许多Go程序对其标准接口的运用，与对其自定义接口的运用同样频繁 。 最后，我们将研究类型断言（type assertions）（§7.10）和类型分支（type switches）（§7.13），并了解它们如何促成一种不同类型的通用性（generality） 。
...
到目前为止，我们所考察的所有类型都是具体类型（concrete types） 。 具体类型指定了其值的确切表示（exact representation），并公开了该表示的固有操作（intrinsic operations），例如数字的算术运算，或切片的索引、追加（append）和遍历（range） 。 具体类型也可以通过其方法提供额外的行为 。 当你拥有一个具体类型的值时，你就确切地知道它是什么以及你能用它做什么 。

Go语言中还有另一种类型，称为接口类型（interface type）。 接口是一种抽象类型（abstract type） 。 它不暴露其值的表示或内部结构，也不暴露它们支持的基本操作集；它仅仅揭示了它们的某些方法 。 当你拥有一个接口类型的值时，你对它是什么（what it is）一无所知； 你只知道它能做什么（what it can do），或者更准确地说，它的方法提供了哪些行为 。
...
在本书中，我们一直在使用两个相似的函数进行字符串格式化：`fmt.Printf`，它将结果写入标准输出（一个文件）；以及`fmt.Sprintf`，它将结果作为字符串返回 。 如果因为这些在使用结果上的表面差异（superficial differences），而不得不重复格式化结果这个困难的部分，那将是十分遗憾的 。 得益于接口，这并没有发生 。 这两个函数实际上都是第三个函数`fmt.Fprintf`的封装（wrappers），`fmt.Fprintf`对其计算结果的处理方式是无关乎具体细节的（agnostic） 。
...
`Fprintf`的`F`前缀代表文件（file），并指明格式化的输出应被写入作为第一个参数提供的文件 。 在`Printf`的例子中，参数`os.Stdout`是一个`*os.File` 。 然而，在`Sprintf`的例子中，参数并非文件，尽管它表面上看起来相似：`&buf`是一个指向内存缓冲区的指针，字节可以被写入该缓冲区 。
`Fprintf`的第一个参数也不是文件。 它是一个`io.Writer`，这是一个接口类型，其声明如下 ：
...
`io.Writer`接口定义了`Fprintf`与其调用者之间的合约（contract） 。 一方面，该合约要求调用者提供一个具体类型（如`*os.File`或`*bytes.Buffer`）的值，该类型必须拥有一个名为`Write`且具有适当签名和行为的方法 。 另一方面，该合约保证`Fprintf`在给定任何满足`io.Writer`接口的值时都能完成其工作 。 `Fprintf`可能不会假定它正在写入文件或内存，只假定它可以调用`Write`方法 。
...
由于`fmt.Fprintf`对值的表示不作任何假设，并且仅依赖于`io.Writer`合约所保证的行为，因此我们可以安全地将任何满足`io.Writer`的具体类型的值作为第一个参数传递给`fmt.Fprintf` 。 这种为一个类型替换另一个满足相同接口的类型的自由称为**可替换性**（substitutability），这是面向对象编程的一个标志（hallmark） 。
...
让我们用一个新的类型来验证这一点。 下面`*ByteCounter`类型的`Write`方法仅仅计算写入它的字节数，然后将其丢弃 。 （需要类型转换以确保`len(p)`和`*c`在`+=`赋值语句中类型匹配。）
...
由于`*ByteCounter`满足`io.Writer`合约，我们可以将其传递给`Fprintf`，`Fprintf`会进行字符串格式化而对这种变化一无所知（oblivious）； `ByteCounter`会正确地累积结果的长度 。
...
除了`io.Writer`，`fmt`包中还有另一个至关重要的接口 。 `Fprintf`和`Fprintln`为类型提供了一种控制其值如何被打印的方式 。 在2.5节中，我们为`Celsius`类型定义了一个`String`方法，使得温度可以打印为 "100°C"；在6.5节中，我们为`*IntSet`配备了一个`String`方法，以便集合可以使用传统的集合表示法（如 "{1 2 3}"）进行渲染 。 声明一个`String`方法会使一个类型满足所有接口中应用最广泛的接口之一：`fmt.Stringer` ：
...
我们将在§7.10解释`fmt`包如何发现哪些值满足此接口 。

练习 7.1：借鉴ByteCounter的思想，实现单词计数器和行计数器。 你会发现bufio.ScanWords很有用 。
练习 7.2：编写一个具有以下签名的函数CountingWriter，它接收一个io.Writer，返回一个新的Writer，该Writer封装了原始的Writer，并返回一个指向int64类型变量的指针，该变量在任何时刻都记录了写入新Writer的字节数 。
`func CountingWriter(w io.Writer) (io.Writer, *int64)`
练习 7.3：为gopl.io/ch4/treesort (§4.4)中的*tree类型编写一个String方法，用于显示树中值的序列 。
...
一个接口类型指定了一组方法，具体类型必须拥有这些方法才能被认为是该接口的一个实例 。 `io.Writer`类型是使用最广泛的接口之一，因为它提供了对所有可以写入字节的类型的抽象，这些类型包括文件、内存缓冲区、网络连接、HTTP客户端、归档器（archivers）、哈希器（hashers）等等 。 `io`包定义了许多其他有用的接口。 `Reader`代表任何可以从中读取字节的类型，而`Closer`是任何可以关闭的值，例如文件或网络连接 。 （到目前为止，你可能已经注意到Go语言中许多单方法接口的命名约定。）
...
更进一步，我们会发现将现有接口组合起来声明新的接口类型 。 这里有两个例子：
...
上面使用的语法，类似于结构体嵌入（struct embedding），允许我们通过指定另一个接口的名称来作为写出其所有方法的简写。 这被称为**接口嵌入**（embedding an interface） 。 我们本可以不使用嵌入来编写`io.ReadWriter`，尽管会不那么简洁，像这样：
...
所有这三个声明具有相同的效果 。 方法出现的顺序无关紧要（immaterial）； 重要的是方法的集合 。

练习 7.4：strings.NewReader函数返回一个通过从其参数（一个字符串）读取数据来满足io.Reader接口（以及其他接口）的值 。 请自行实现一个NewReader的简单版本，并用它来使HTML解析器（§5.2）从字符串接收输入 。
练习 7.5：io包中的LimitReader函数接受一个io.Reader r和一个字节数n，并返回另一个Reader，该Reader从r读取数据，但在读取n个字节后报告文件结束（end-of-file）条件 。 请实现它 。
`func LimitReader(r io.Reader, n int64) io.Reader`
...
如果一个类型拥有接口所要求的所有方法，那么它就满足该接口 。 例如，一个`*os.File`满足`io.Reader`、`Writer`、`Closer`和`ReadWriter` 。 一个`*bytes.Buffer`满足`Reader`、`Writer`和`ReadWriter`，但不满足`Closer`，因为它没有`Close`方法 。 作为一种简写，Go程序员常说一个具体类型“是”某个特定的接口类型，意指它满足该接口 。 例如，一个`*bytes.Buffer`是一个`io.Writer`；一个`*os.File`是一个`io.ReadWriter` 。
...
```
接口的赋值规则（§2.4.2）非常简单：一个表达式只有当其类型满足接口时才能赋值给该接口 。 所以：
```go
var w io.Writer
...
由于`ReadWriter`和`ReadWriteCloser`包含了`Writer`的所有方法，任何满足`ReadWriter`或`ReadWriteCloser`的类型必然满足`Writer` 。
...
在我们进一步讨论之前，我们应该阐释一个类型拥有一个方法的含义中的一个微妙之处（subtlety） 。 回想一下第6.2节，对于每个已命名的具体类型`T`，它的一些方法接收者是类型`T`本身，而另一些则需要一个`*T`指针 。 还要回想一下，只要参数是一个变量（variable），就可以在类型为`T`的参数上调用`*T`的方法； 编译器会隐式地获取其地址 。 但这仅仅是语法糖：类型`T`的值并不拥有`*T`指针所拥有的所有方法，因此它可能满足更少的接口 。
...
```
一个例子将清楚地阐明这一点。 第6.5节中`IntSet`类型的`String`方法需要一个指针接收者，所以我们不能在一个不可寻址的（non-addressable）`IntSet`值上调用该方法：
```go
type IntSet struct { /*...*/ }
func (*IntSet) String() string
// var _ = IntSet{}.String() // compile error: String requires *IntSet receiver
...
```
然而，由于只有`*IntSet`拥有`String`方法，因此只有`*IntSet`满足`fmt.Stringer`接口：
```go
var _ fmt.Stringer = &s // OK
...
第12.8节包含一个打印任意值的方法的程序，而`godoc -analysis type`工具（§10.7.4）则显示了每种类型的方法以及接口和具体类型之间的关系 。
...
```
就像一个信封包装并隐藏了它所装的信件一样，接口包装并隐藏了它所持有的具体类型和值 。 只有接口类型所暴露的方法可以被调用，即使具体类型拥有其他方法：
```go
// os.Stdout.Write([]byte("hello")) // OK: *os.File has Write method
// os.Stdout.Close()                // OK: *os.File has Close method
...
```
一个拥有更多方法的接口，例如`io.ReadWriter`，比一个拥有较少方法的接口（例如`io.Reader`）能告诉我们更多关于它所包含的值的信息，并且对实现它的类型提出了更高的要求 。 那么，没有任何方法的`interface{}`类型，它能告诉我们关于满足它的具体类型的什么信息呢？ 没错：什么也没有（nothing） 。 这看起来可能毫无用处，但实际上，被称为**空接口类型**（empty interface type）的`interface{}`是不可或缺的（indispensable） 。 因为空接口类型对其满足的类型没有任何要求，所以我们可以将任何值赋给空接口 。
```go
var any interface{}
any = true
any = 12.34
...
虽然不那么明显，但从本书的第一个例子开始，我们就一直在使用空接口类型，因为它允许像`fmt.Println`这样的函数，或者第5.7节中的`errorf`函数，接受任何类型的参数 。 当然，一旦我们创建了一个包含布尔值、浮点数、字符串、map、指针或任何其他类型的`interface{}`值，由于该接口没有任何方法，我们无法直接对其持有的值进行任何操作 。 我们需要一种方法将其值取回（get the value back out again） 。 我们将在§7.10中看到如何使用类型断言来实现这一点 。
...
```
由于接口的满足仅取决于所涉及的两种类型的方法，因此无需声明具体类型与其满足的接口之间的关系 。 尽管如此，当这种关系是预期的但程序并未以其他方式强制执行时，记录并断言这种关系有时是很有用的 。 下面的声明在编译时断言`*bytes.Buffer`类型的值满足`io.Writer`：
```go
// *bytes.Buffer must satisfy io.Writer
var w io.Writer = new(bytes.Buffer)
```
...
```
我们不必分配一个新的变量，因为任何`*bytes.Buffer`类型的值都可以，即使是`nil`，我们使用显式转换将其写作`(*bytes.Buffer)(nil)` 。 而且由于我们从不打算引用`w`，我们可以用空白标识符替换它 。 综合这些改变，我们得到了这个更为 frugal（节俭的，简洁的）变体：
```go
// *bytes.Buffer must satisfy io.Writer
var _ io.Writer = (*bytes.Buffer)(nil)
```
...
像`io.Writer`这样的非空接口类型通常由指针类型来满足，特别是当一个或多个接口方法暗示对接收者进行某种修改（mutation）时，正如`Write`方法所做的那样 。 指向结构体的指针是一种特别常见的拥有方法的类型（method-bearing type） 。 但指针类型绝不是唯一满足接口的类型，即使是带有修改器方法（mutator methods）的接口也可能由Go的其他引用类型满足 。 我们已经见过带有方法的切片类型（`geometry.Path`，§6.1）和带有方法的map类型（`url.Values`，§6.2.1）的例子，稍后我们将看到一个带有方法的函数类型（`http.HandlerFunc`，§7.7） 。 即使是基本类型也可以满足接口；正如我们在§7.4看到的，`time.Duration`满足`fmt.Stringer` 。
...
```
一个具体类型可以满足许多不相关的接口 。 考虑一个组织或销售数字化文化制品（如音乐、电影和书籍）的程序 。 它可能会定义以下具体类型集合：
Album Book Movie Magazine Podcast TVEpisode Track
我们可以将每个感兴趣的抽象表示为一个接口 。 某些属性是所有制品共有的，例如标题、创作日期和创作者列表（作者或艺术家） 。
```go
type Artifact interface {
    Title() string
    Creators() []string
...
```
其他属性则仅限于特定类型的制品 。 印刷品的属性仅与书籍和杂志相关，而只有电影和电视剧集才有屏幕分辨率 。
```go
type Text interface {
    Pages() int
    Words() int
...
```
这些接口只是将相关的具体类型组合在一起并表达它们共同特性（facets）的一种有用方式 。 我们稍后可能会发现其他分组 。 例如，如果我们发现需要以相同的方式处理`Audio`和`Video`项目，我们可以定义一个`Streamer`接口来表示它们的共同方面，而无需更改任何现有的类型声明 。
```go
type Streamer interface {
    Stream() (io.ReadCloser, error)
    RunningTime() time.Duration
...
每个基于共享行为的具体类型的分组都可以表示为一个接口类型 。 与基于类的语言（其中一个类满足的接口集合是显式的）不同，在Go中，我们可以在需要时定义新的抽象或感兴趣的分组，而无需修改具体类型的声明 。 当具体类型来自不同作者编写的包时，这尤其有用 。 当然，具体类型之间确实需要存在底层的共性（commonalities） 。
...
```
在本节中，我们将看到另一个标准接口`flag.Value`如何帮助我们为命令行参数（command-line flags）定义新的表示法（notations） 。 考虑下面的程序，它会休眠指定的时间 。
_gopl.io/ch7/sleep_
```go
var period = flag.Duration("period", 1*time.Second, "sleep period")
...
Sleeping for 1s...
```
在进入休眠之前，它会打印时间周期 。 `fmt`包调用`time.Duration`的`String`方法来打印周期，不是以纳秒为单位的数字，而是以用户友好的表示法：
```bash
$ go build gopl.io/ch7/sleep
$ ./sleep
Sleeping for 1s...
...
```
默认情况下，休眠周期是一秒，但可以通过`-period`命令行参数进行控制 。 `flag.Duration`函数创建一个`time.Duration`类型的参数变量，并允许用户以多种用户友好的格式指定持续时间，包括`String`方法打印的相同表示法 。 这种设计的对称性（symmetry of design）带来了良好的用户界面 。
```bash
$ ./sleep -period 50ms
Sleeping for 50ms...
$ ./sleep -period 2m30s
...
```
因为表示持续时间的参数非常有用，这个功能被内置到了`flag`包中，但为我们自己的数据类型定义新的参数表示法也很容易 。 我们只需要定义一个满足`flag.Value`接口的类型，其声明如下：
```go
package flag
...
`String`方法格式化参数的值，用于命令行帮助信息； 因此，每个`flag.Value`也是一个`fmt.Stringer` 。 `Set`方法解析其字符串参数并更新参数值 。 实际上，`Set`方法是`String`方法的逆过程，良好的实践是让它们使用相同的表示法 。
...
```
让我们定义一个`celsiusFlag`类型，允许以摄氏度指定温度，或者以华氏度指定并进行适当转换 。 注意`celsiusFlag`嵌入了一个`Celsius`类型（§2.5），从而免费获得了`String`方法 。 为了满足`flag.Value`接口，我们只需要声明`Set`方法：
_gopl.io/ch7/tempconv_
```go
// *celsiusFlag satisfies the flag.Value interface.
type celsiusFlag struct{ Celsius }
...
```
对`fmt.Sscanf`的调用从输入字符串`s`中解析一个浮点数(`value`)和一个字符串(`unit`) 。 虽然通常必须检查`Sscanf`的错误结果，但在这种情况下我们不需要，因为如果存在问题，`switch`的任何一个`case`都不会匹配 。 下面的`CelsiusFlag`函数将所有这些封装起来 。 它向调用者返回一个指向嵌入在`celsiusFlag`变量`f`中的`Celsius`字段的指针 。 `Celsius`字段是在参数处理期间将由`Set`方法更新的变量 。 对`Var`的调用将该参数添加到应用程序的命令行参数集，即全局变量`flag.CommandLine` 。 具有异常复杂命令行界面的程序可能拥有此类型的多个变量 。 对`Var`的调用将一个`*celsiusFlag`参数赋给一个`flag.Value`形参，这会使编译器检查`*celsiusFlag`是否具有必要的方法 。
```go
// CelsiusFlag defines a Celsius flag with the specified name,
// default value, and usage, and returns the address of the flag variable.
// The flag argument must have a quantity and a unit, e.g., "100C".
...
从概念上讲，接口类型的值，即**接口值**（interface value），有两个组成部分：一个具体类型（concrete type）和该类型的一个值 。 这两个部分分别被称为接口的**动态类型**（dynamic type）和**动态值**（dynamic value） 。
...
对于像Go这样的静态类型语言，类型是编译时的概念，因此类型不是一个值 。 在我们的概念模型中，一组称为**类型描述符**（type descriptors）的值提供了关于每种类型的信息，例如其名称和方法 。 在接口值中，类型组件由相应的类型描述符表示 。
...
w = nil
```
在下面的四个语句中，变量`w`依次取了三个不同的值（初始值和最终值相同） 。
```go
var w io.Writer
w = os.Stdout
w = new(bytes.Buffer)
...
让我们仔细观察每个语句执行后`w`的值和动态行为 。 第一个语句声明了`w`：
`var w io.Writer`
在Go中，变量总是被初始化为一个明确定义的值，接口也不例外 。 接口的零值，其类型和值组件都设置为`nil` (图 7.1) 。
```
w
类型 (type)   值 (value)
nil           nil
...
一个接口值根据其动态类型是否为`nil`来判断其自身是否为`nil`或非`nil`。 因此，这是一个`nil`接口值 。 你可以使用`w == nil`或`w != nil`来测试一个接口值是否为`nil` 。 调用一个`nil`接口值的任何方法都会导致 panic：
`w.Write([]byte("hello")) // panic: nil pointer dereference`
...
第二个语句将一个*os.File类型的值赋给w：
`w = os.Stdout`
此赋值涉及从具体类型到接口类型的隐式转换，等价于显式转换`io.Writer(os.Stdout)` 。 这种转换，无论是显式的还是隐式的，都会捕获其操作数的类型和值 。 接口值的动态类型被设置为指针类型`*os.File`的类型描述符，其动态值持有`os.Stdout`的副本，`os.Stdout`是一个指向代表进程标准输出的`os.File`变量的指针 (图 7.2) 。
```
w
类型 (type)      值 (value)
*os.File         (指向 os.File 结构体的指针，该结构体代表 fd=1 即标准输出)
```
图 7.2. 一个包含*os.File指针的接口值。
...
在一个包含*os.File指针的接口值上调用Write方法，会导致(*os.File).Write方法被调用 。 该调用会打印 "hello" 。
`w.Write([]byte("hello")) // "hello"`
通常，我们在编译时无法知道接口值的动态类型是什么，因此通过接口进行的调用必须使用**动态派发**（dynamic dispatch） 。 编译器不会生成直接调用，而是必须生成代码以从类型描述符中获取名为`Write`的方法的地址，然后对该地址进行间接调用 。 调用的接收者参数是接口动态值`os.Stdout`的副本 。 其效果就如同我们直接进行了如下调用：
`os.Stdout.Write([]byte("hello")) // "hello"`
...
`w = new(bytes.Buffer)`
动态类型现在是`*bytes.Buffer`，动态值是指向新分配的缓冲区的指针 (图 7.3) 。
```
w
类型 (type)         值 (value)
*bytes.Buffer       (指向新分配的 bytes.Buffer 结构体的指针)
```
图 7.3. 一个包含*bytes.Buffer指针的接口值。
...
对`Write`方法的调用使用与之前相同的机制：
`w.Write([]byte("hello")) // writes "hello" to the bytes.Buffer`
这一次，类型描述符是`*bytes.Buffer`，所以`(*bytes.Buffer).Write`方法被调用，缓冲区的地址作为接收者参数的值 。 该调用将 "hello" 追加到缓冲区 。
...
最后，第四条语句将nil赋给接口值：
`w = nil`
这将它的两个组件都重置为`nil`，使`w`恢复到声明时的状态，如图 7.1 所示 。
...
接口值可以持有任意大的动态值 。 例如，代表时间瞬间的time.Time类型是一个包含若干未导出字段的结构体类型 。 如果我们用它创建一个接口值：
`var x interface{} = time.Now()`
结果可能如图7.4所示 。 从概念上讲，无论其类型有多大，动态值总是能容纳在接口值之内 。 （这只是一个概念模型；实际的实现则大相径庭。）
```
x
类型 (type)     值 (value)
time.Time       (time.Time 结构体，包含如 sec, nsec, loc 等字段)
```
图 7.4. 一个持有time.Time结构体的接口值。
...
```
接口值可以使用`==`和`!=`进行比较 。 两个接口值相等，当且仅当它们都为`nil`，或者它们的动态类型相同且它们的动态值根据该类型的`==`的通常行为也相等 。 由于接口值是可比较的，它们可以用作map的键或`switch`语句的操作数 。 然而，如果比较两个动态类型相同但该类型本身不可比较（例如，切片）的接口值，则比较操作会引发 panic：
```go
var x interface{} = []int{1, 2, 3}
fmt.Println(x == x) // panic: comparing uncomparable type []int
```
...
在这方面，接口类型比较特殊（unusual） 。 其他类型要么是安全可比较的（如基本类型和指针），要么是完全不可比较的（如切片、map和函数），但是在比较接口值或包含接口值的聚合类型时，我们必须意识到潜在的panic风险 。 当使用接口作为map键或`switch`操作数时，也存在类似的风险 。 仅当确信接口值包含可比较类型的动态值时，才应比较它们 。
...
fmt.Printf("%T\n", w) // "*bytes.Buffer"
```
在处理错误或调试时，报告接口值的动态类型通常很有帮助 。 为此，我们使用`fmt`包的`%T`格式化动词：
```go
var w io.Writer
fmt.Printf("%T\n", w) // "<nil>"
w = os.Stdout
...
在内部，`fmt`使用反射来获取接口动态类型的名称 。 我们将在第12章探讨反射 。
...
一个`nil`接口值（完全不包含任何值）与一个包含碰巧为`nil`的指针的接口值是不同的 。 这种微妙的区别（subtle distinction）是一个常见的陷阱（trap），几乎每个Go程序员都曾遇到过 。
...
```
考虑下面的程序。 当`debug`设置为`true`时，`main`函数将函数`f`的输出收集到一个`bytes.Buffer`中 。
```go
const debug = true
...
// }
```
我们可能期望将`debug`改为`false`会禁用输出收集，但实际上它会导致程序在`out.Write`调用期间发生panic：
```go
// if out != nil {
//     out.Write([]byte("done!\n")) // panic: nil pointer dereference
// }
...
当`main`调用`f`时，它将一个类型为`*bytes.Buffer`的`nil`指针赋给了`out`参数，因此`out`的动态值是`nil` 。 然而，其动态类型是`*bytes.Buffer`，这意味着`out`是一个**非nil的接口，其内部包含一个nil指针值**（图 7.5），所以防御性检查`out != nil`仍然为真 。
```
out
类型 (type)      值 (value)
*bytes.Buffer    nil
```
_图 7.5. 一个包含nil指针的非nil接口。_
...
与之前一样，动态派发机制确定必须调用`(*bytes.Buffer).Write`方法，但这次的接收者值是`nil` 。 对于某些类型，例如`*os.File`，`nil`是一个有效的接收者（§6.2.1），但`*bytes.Buffer`并非如此 。 该方法被调用，但在尝试访问缓冲区时会发生panic 。
...
f(buf) // OK
```
问题在于，尽管一个`nil`的`*bytes.Buffer`指针拥有满足该接口所需的方法，但它并不满足该接口的行为要求（behavioral requirements） 。 特别是，该调用违反了`(*bytes.Buffer).Write`的隐式前提条件（implicit precondition），即其接收者不为`nil`，因此将`nil`指针赋给该接口是一个错误 。 解决方案是将`main`函数中`buf`的类型更改为`io.Writer`，从而从一开始就避免将这个功能失常的（dysfunctional）值赋给接口：
```go
var buf io.Writer
if debug {
    buf = new(bytes.Buffer) // enable collection of output
...
既然我们已经介绍了接口值的机制，接下来让我们看一些Go标准库中更重要的接口 。 在接下来的三个小节中，我们将看到接口如何用于排序、Web服务和错误处理 。